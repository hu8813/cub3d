...
{
	if (!ft_strnstr(buffer, "NO ", ft_strlen(buffer)) || !ft_strnstr(buffer,
			"SO ", ft_strlen(buffer)) || !ft_strnstr(buffer, "WE ",
			ft_strlen(buffer)) || !ft_strnstr(buffer, "EA ", ft_strlen(buffer))
		|| !ft_strnstr(buffer, "C ", ft_strlen(buffer)) || !ft_strnstr(buffer,
			"F ", ft_strlen(buffer)))
		return (0);
	return (1);
}
...
static void	map_split(char *buffer, t_main *main)
{
	...
	if (main->floor[0] == -1 || main->ceil[0] == -1 || main->floor[1] == -1
		|| main->ceil[1] == -1 || main->floor[2] == -1 || main->ceil[2] == -1
		|| !main->north || !main->south || !main->west || !main->east)
		ft_error("Error\nMissing texture or color", main);
	if (buffer[i - 1] != '\n')
		height++;
	i = 0;
